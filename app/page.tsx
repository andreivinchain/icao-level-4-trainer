...
import { audioSrc, courseTests, lessons, type CourseLesson, type CourseTest } from "./course-data";
...
function Lesson({lesson,completed,onComplete,back,audio,progress,setProgress}:{lesson:CourseLesson;completed:boolean;onComplete:()=>void;back:()=>void;audio:boolean;progress:Progress;setProgress:React.Dispatch<React.SetStateAction<Progress>>}) {
  ...
  const tasks = [
    {title:"Describe the picture",tag:"SPEAKING",body:`Опишите сцену по ключевым словам: ${lesson.pictureWords.join(", ")}. Отделяйте наблюдение от предположения.`},
    {title:"Picture questions",tag:"INTERACTION",body:`Ответьте без подготовки: где происходит событие, что к нему привело, какой риск возникает и что произойдёт дальше?`},
    {title:"Sample answer",tag:`LISTENING ${lesson.cd}.${lesson.tracks[0]}`,body:"Прослушайте образец. Отметьте три точных описания положения объектов и две формы предположения."},
    {title:"Plain English - gist",tag:`LISTENING ${lesson.cd}.${lesson.tracks[1]}`,body:`Прослушайте основную запись по теме ${lesson.title}. Сформулируйте главную мысль одним предложением.`},
    {title:"Plain English - detail",tag:`LISTENING ${lesson.cd}.${lesson.tracks[1]}`,body:"Прослушайте ещё раз. Запишите последовательность событий, числа, причины и принятое решение."},
    {title:"Radiotelephony - listening",tag:`R/T ${lesson.tracks.slice(2).map(t=>`${lesson.cd}.${t}`).join("-")}`,body:`Фокус: ${lesson.radioFocus}. Определите проблему, запрос, инструкцию, readback и итог.`},
    {title:"Radiotelephony - Plain English",tag:"STRUCTURE",body:`Восстановите и произнесите фразы: ${lesson.phrases.join(" / ")}`},
    {title:"Clarification role-play",tag:"INTERACTIONS",body:"Разыграйте Pilot ↔ ATC. Первый участник сообщает проблему, второй уточняет неполную информацию; затем поменяйтесь ролями."},
    {title:"Vocabulary Check",tag:"VOCABULARY",body:`Объясните по-английски и используйте в контексте: ${lesson.words.map(w=>w[0]).join(", ")}.`},
    {title:"Discussion",tag:"FLUENCY",body:`Говорите 90 секунд: причины, последствия, предотвращение и будущие технологии применительно к теме “${lesson.title}”.`},
  ];
  const toggleTask=(id:string)=>setProgress(p=>({...p,exercises:p.exercises.includes(id)?p.exercises.filter(x=>x!==id):[...p.exercises,id]}));
  const done=tasks.filter((_,i)=>progress.exercises.includes(`${lesson.id}-e${i+1}`)).length;
  return <section className="page narrow"><button className="back" onClick={back}>← К полному курсу</button><div className="lesson-hero"><span>{lesson.n}</span><div><em>Книга, стр. {lesson.pages} · CD{lesson.cd}: {lesson.tracks.map(t=>`${lesson.cd}.${t}`).join(", ")}</em><h2>{lesson.title}</h2><p>{lesson.objective}</p></div></div>
    ...
    {audio && <div className="audio-playlist">{lesson.tracks.map((track,i)=><div className="audio-block" key={track}><div><span>▶</span><div><b>{lesson.cd}.{track}</b><small>{i===0?"Sample answer":i===1?"Plain English":"Radiotelephony"}</small></div></div><audio controls preload="none" src={audioSrc(lesson.cd,track)}><track kind="captions" srcLang="en" src="/audio/captions-note.vtt" label="Listening Script"/></audio></div>)}</div>}
    <div className="exercise-list">{tasks.map((task,i)=>{const id=`${lesson.id}-e${i+1}`;const checked=progress.exercises.includes(id);return <article className={checked?"exercise done":"exercise"} key={id}><button className="exercise-check" onClick={()=>toggleTask(id)} aria-label={checked?"Отменить выполнение":"Отметить выполненным"}>{checked?"✓":String(i+1).padStart(2,"0")}</button><div><em>{task.tag}</em><h3>{task.title}</h3><p>{task.body}</p>{i===8&&<div className="word-chips">{lesson.words.map(([en,ru])=><span key={en}><b>{en}</b>{ru}</span>)}</div>}{i===9&&<textarea value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="Write brief notes in English…"/>}</div></article>})}</div>
    <div className="complete-unit"><span>{answer.trim().split(/\s+/).filter(Boolean).length} слов в заметках</span><button className={completed?"secondary complete":"primary"} onClick={onComplete}>{completed?"✓ Unit завершён":"Завершить unit"}</button></div><SourceNote /></section>
}
...
function CourseTestPage({test,progress,setProgress,audio,back}:{test:CourseTest;progress:Progress;setProgress:React.Dispatch<React.SetStateAction<Progress>>;audio:boolean;back:()=>void}) {
  ...
  return <section className="page narrow"><button className="back" onClick={back}>← К списку тестов</button><div className="lesson-hero test-hero"><span>✓</span><div><em>Книга, стр. {test.pages} · {test.scope}</em><h2>{test.title}</h2><p>Контрольная работа по структуре книги: picture, Plain English, radiotelephony, clarification, vocabulary и discussion.</p></div></div>{audio&&<div className="audio-playlist">{test.tracks.map((track,i)=><div className="audio-block" key={track}><div><span>▶</span><div><b>{test.cd}.{track}</b><small>{i===0?"Sample answer":i===1?"Plain English":"Radiotelephony"}</small></div></div><audio controls preload="none" src={audioSrc(test.cd,track)}><track kind="captions" srcLang="en" src="/audio/captions-note.vtt" label="Listening Script"/></audio></div>)}</div>}<div className="content-card"><span className="eyebrow">ЧЕК-ЛИСТ ТЕСТА</span>{activities.map((x,i)=><label className="test-activity" key={x}><input type="checkbox"/> <b>{i+1}.</b> {x}</label>)}</div><div className="content-card"><span className="eyebrow">SELF-ASSESSMENT</span><p>Оцените Pronunciation, Structure, Vocabulary, Fluency, Comprehension и Interactions. Это самооценка, а не официальный результат ICAO.</p><button className={complete?"secondary complete":"primary"} onClick={toggle}>{complete?"✓ Тест завершён":"Завершить тест"}</button></div></section>
}

function Vocabulary({progress,setProgress,startCards}:{progress:Progress;setProgress:React.Dispatch<React.SetStateAction<Progress>>;startCards:()=>void}) { const [q,setQ]=useState(""); const filtered=vocab.filter(v=>(v.term+v.tr+v.cat).toLowerCase().includes(q.toLowerCase())); return <section className="page"><div className="section-heading"><div><span className="eyebrow">ACTIVE VOCABULARY</span><h2>Авиационная лексика</h2></div><button className="primary" onClick={startCards}>Режим карточек →</button></div><div className="toolbar"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Поиск по термину или категории…"/><span>{progress.words.length}/{vocab.length} изучено</span></div><div className="vocab-grid">{filtered.map(v=><article key={v.id} className={progress.words.includes(v.id)?"learned":""}><div><em>{v.cat}</em><button onClick={()=>setProgress(p=>({...p,words:p.words.includes(v.id)?p.words.filter(x=>x!==v.id):[...p.words,v.id]}))}>{progress.words.includes(v.id)?"✓":"○"}</button></div><h3>{v.term}</h3><b>{v.tr}</b><p>{v.context}</p><blockquote>“{v.example}”</blockquote></article>)}</div><SourceNote /></section> }

function Cards({card,flipped,setFlipped,progress,setProgress,next}:{card:number;flipped:boolean;setFlipped:(x:boolean)=>void;progress:Progress;setProgress:React.Dispatch<React.SetStateAction<Progress>>;next:()=>void}) { const v=vocab[card]; return <section className="page narrow flash-page"><span className="eyebrow">КАРТОЧКА {card+1} ИЗ {vocab.length}</span><button type="button" className={flipped?"flashcard flipped":"flashcard"} onClick={()=>setFlipped(!flipped)}><small>{v.cat}</small><h2>{flipped?v.tr:v.term}</h2><p>{flipped?v.context:"Нажмите, чтобы увидеть перевод"}</p>{flipped&&<blockquote>“{v.example}”</blockquote>}</button><div className="flash-actions"><button onClick={next}>Повторить</button><button className="primary" onClick={()=>{setProgress(p=>({...p,words:p.words.includes(v.id)?p.words:[...p.words,v.id]}));next()}}>Знаю →</button></div><p className="center-note">Отмечено изученными: {progress.words.length}</p></section> }
