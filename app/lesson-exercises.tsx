...
type LessonDraft = {
  ...
  gistChoice: number|null;
  detailNotes: Record<string,string>;
  radioNotes: Record<string,string>;
  rolePlay: string;
  discussion: string;
};
...
function VocabularyMatch({lesson}:{lesson:CourseLesson}) {
  ...
  const storageKey=`icao-vocabulary-match-${lesson.id}`;
  useEffect(()=>{
    try {
      const saved=localStorage.getItem(storageKey);
      if(saved) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate device-local exercise answers
        setAnswers(JSON.parse(saved));
      }
    } catch { localStorage.removeItem(storageKey) }
    setReady(true);
  },[storageKey]);
  useEffect(()=>{if(ready)localStorage.setItem(storageKey,JSON.stringify(answers))},[answers,ready,storageKey]);
...
export default function LessonExercises({lesson,audio,completedIds,onToggle}:LessonExercisesProps) {
  ...
  const blankDraft=useMemo<LessonDraft>(()=>({
    description:"",
    questionAnswers:content.pictureQuestions.map(()=>""),
    gistChoice:null,
    detailNotes:{},
    radioNotes:{},
    rolePlay:"",
    discussion:"",
  }),[content.pictureQuestions]);
  const [draft,setDraft]=useState<LessonDraft>(blankDraft);
  const [draftReady,setDraftReady]=useState(false);
  const draftKey=`icao-lesson-draft-${lesson.id}`;
  useEffect(()=>{
    try {
      const saved=localStorage.getItem(draftKey);
      const parsed=saved?JSON.parse(saved):{};
      const questionAnswers=Array.isArray(parsed.questionAnswers)
        ?content.pictureQuestions.map((_,index)=>parsed.questionAnswers[index]??"")
        :blankDraft.questionAnswers;
      // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate device-local lesson answers
      setDraft({...blankDraft,...parsed,questionAnswers});
    } catch {
      localStorage.removeItem(draftKey);
      setDraft(blankDraft);
    }
    setDraftReady(true);
  },[blankDraft,content.pictureQuestions,draftKey]);
  useEffect(()=>{if(draftReady)localStorage.setItem(draftKey,JSON.stringify(draft))},[draft,draftKey,draftReady]);
  ...
  return <div className="exercise-list rich-exercises">
    <div className="draft-status">✓ Ответы автоматически сохраняются на этом устройстве</div>
    <ExerciseCard id={`${lesson.id}-e1`} number={1} tag="SPEAKING · BOOK 1A" title="Describe the picture" checked={isDone(1)} onToggle={onToggle}>
      ...
      <textarea value={draft.description} onChange={event=>setDraft({...draft,description:event.target.value})} placeholder="Write or dictate your description in English…"/>
      <div className="response-meta"><span>{draft.description.trim().split(/\s+/).filter(Boolean).length} words</span><button className="reveal" onClick={()=>toggleModel(1)}>{showModel[1]?"Скрыть образец":"Показать sample answer"}</button></div>
      <VoiceRecorder storageKey={`${lesson.id}-picture-description`}/>
    ...
    <ExerciseCard id={`${lesson.id}-e2`} number={2} tag="INTERACTION · BOOK 1B" title="Picture questions" checked={isDone(2)} onToggle={onToggle}>
      <div className="picture-question-layout"><img src={content.image} alt="Reference scene for the questions"/><div>{content.pictureQuestions.map((question,index)=><label key={question}><span><b>{index+1}</b>{question}</span><input value={draft.questionAnswers[index]} onChange={event=>setDraft({...draft,questionAnswers:draft.questionAnswers.map((value,i)=>i===index?event.target.value:value)})} placeholder="Short answer in English…"/></label>)}</div></div>
      <div className="response-meta"><span>{draft.questionAnswers.filter(value=>value.trim()).length}/5 answered</span><button className="reveal" onClick={()=>toggleModel(2)}>{showModel[2]?"Скрыть опору":"Показать answer frame"}</button></div>
    ...
    <ExerciseCard id={`${lesson.id}-e4`} number={4} tag={`LISTENING ${lesson.cd}.${lesson.tracks[1]} · BOOK 2A`} title="Plain English — listening for gist" checked={isDone(4)} onToggle={onToggle}>
      ...
      <div className="quiz-options">{gistOptions.map((option,index)=><button className={draft.gistChoice===index?(index===correctPosition?"selected correct":"selected incorrect"):""} onClick={()=>setDraft({...draft,gistChoice:index})} key={option}><span>{String.fromCharCode(65+index)}</span>{option}</button>)}</div>
      {draft.gistChoice!==null&&<p className={draft.gistChoice===correctPosition?"feedback good":"feedback bad"}>{draft.gistChoice===correctPosition?"✓ Correct — this is the main operational point.":"Not quite. Listen for the problem, action and outcome."}</p>}
    </ExerciseCard>
    ...
    <ExerciseCard id={`${lesson.id}-e5`} number={5} tag={`LISTENING ${lesson.cd}.${lesson.tracks[1]} · BOOK 2B`} title="Plain English — listening for detail" checked={isDone(5)} onToggle={onToggle}>
      ...
      <div className="notes-grid">{detailFields.map(field=><label key={field}><b>{field}</b><textarea value={draft.detailNotes[field]??""} onChange={event=>setDraft({...draft,detailNotes:{...draft.detailNotes,[field]:event.target.value}})} placeholder="Names, figures, sequence or decision…"/></label>)}</div>
    ...
    <ExerciseCard id={`${lesson.id}-e6`} number={6} tag={`RADIOTELEPHONY · ${lesson.tracks.slice(2).map(track=>`${lesson.cd}.${track}`).join(" / ")}`} title="Radiotelephony — listening" checked={isDone(6)} onToggle={onToggle}>
      ...
      <div className="notes-grid radio-map">{radioFields.map(field=><label key={field}><b>{field}</b><input value={draft.radioNotes[field]??""} onChange={event=>setDraft({...draft,radioNotes:{...draft.radioNotes,[field]:event.target.value}})} placeholder="What exactly was said?"/></label>)}</div>
    ...
    <ExerciseCard id={`${lesson.id}-e8`} number={8} tag="INTERACTIONS · BOOK 4" title="Clarification role-play" checked={isDone(8)} onToggle={onToggle}>
      ...
      <div className="dialogue practice-dialogue"><div><b>ATC</b><p>{lesson.phrases[0]}</p></div><div><b>PILOT</b><textarea value={draft.rolePlay} onChange={event=>setDraft({...draft,rolePlay:event.target.value})} placeholder="Your clarification and readback…"/></div></div>
      <div className="starter-row"><span>Quick inserts:</span>{["Say again…","Confirm…","Unable…","Request…"].map(starter=><button key={starter} onClick={()=>setDraft({...draft,rolePlay:`${draft.rolePlay}${draft.rolePlay?" ":""}${starter}`})}>{starter}</button>)}</div>
      <VoiceRecorder storageKey={`${lesson.id}-clarification`}/>
    ...
    <ExerciseCard id={`${lesson.id}-e10`} number={10} tag="FLUENCY · BOOK 6" title="Discussion — 90-second response" checked={isDone(10)} onToggle={onToggle}>
      ...
      <textarea value={draft.discussion} onChange={event=>setDraft({...draft,discussion:event.target.value})} placeholder="Brief speaking notes in English…"/>
      <VoiceRecorder storageKey={`${lesson.id}-discussion`} showSelfAssessment/>
